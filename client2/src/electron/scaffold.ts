// hacked versions of all these packages so this works in the browser
...
export default class ScaffoldCommunicator {
    ...
    constructor(scaffoldPath: string) {
        ...
        if (!fs.existsSync(this.wrapperPath)) throw new Error(`Can't find gradle wrapper: ${this.wrapperPath}`)
    ...
    get sourcePath() {
        ...
        if (!fs.existsSync(sp)) sp = path.join(this.scaffoldPath, 'example-bots', 'src', 'main')
        return sp
    }
    ...
    static findDefaultScaffoldPath(): string | null {
        ...
        // npm run electron in client, if battlecodeXX-scaffold is located in same level as battlecodeXX
    ...
    static promptForScaffoldPath(): string | null {
        return prompt(
            "We couldn't find the scaffold. Enter the path to the scaffold folder, or press cancel to continue without it"
        )
    }
        ...
        fs.stat(this.mapPath, (err, stat) => {
            ...
            fs.readdir(this.mapPath, (err, files) => {
                ...
                return cb(
                    ...
                    new Set(
                        files
                            .filter((file) => file.endsWith(`map${BATTLECODE_YEAR % 100}`))
                            .map((file) => file.substring(0, file.length - `map${BATTLECODE_YEAR % 100}`.length))
    ...
    saveMap(mapData: Uint8Array, mapName: string, cb: (err: Error | null) => void) {
        ...
        fs.writeFile(
            path.join(this.scaffoldPath, 'maps', mapName + `map${BATTLECODE_YEAR % 100}`),
            Buffer.from(mapData),
            cb
        )
    }
    ...
    runMatch(
        ...
        const proc = child_process.spawn(this.wrapperPath, options, { cwd: this.scaffoldPath })
        ...
        proc.on('pid-message', (event, arg) => {
            console.log('Mystery Code:', arg)
            // this code used to do nothing, this was bound incorrectly. No idea if 
            // this is what they meant or if this code ever even ran
            this.procs.push(arg)
        })
