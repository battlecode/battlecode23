...
export default class ScaffoldCommunicator {
  ...
  static findDefaultScaffoldPath(): string | null {
    ...
    const fromDev = path.join(path.dirname(path.dirname(path.dirname(appPath))), 'battlecode' + config.defaults().year.substring(2) + '-scaffold')
  ...
  getMaps(cb: (err: Error | null, maps?: Set<string>) => void) {
    fs.stat(this.mapPath, (err, stat) => {
      ...
      fs.readdir(this.mapPath, (err, files) => {
        ...
        return cb(null, new Set(files.filter((file) => file.endsWith('.' + config.defaults().map_extension))
          .map((file) => file.substring(0, file.length - ('.' + config.defaults().map_extension).length))
  ...
  saveMap(mapData: Uint8Array, mapName: string, cb: (err: Error | null) => void) {
    ...
    fs.writeFile(path.join(this.scaffoldPath, 'maps', mapName + '.' + config.defaults().map_extension),
      new Buffer(mapData),
      cb)
  }
